...
## Changelog
- **1.1.0 (2026-09-06, hardening)** — `PATCH /transfers/{id}/settle` passa a ler o `settlement_ref` de corpos **chunked** (sem `Content-Length`): decode via `httpx.DecodeOptional` tratando `io.EOF` como body vazio, sem gatear por `ContentLength > 0`. Antes, um cliente que enviasse o corpo chunked (ex.: `HttpClient`/.NET) tinha o `settlement_ref` silenciosamente ignorado e persistido como `NULL` mesmo com `200`. Patch retrocompatível; achado do E2E do Liquida.
